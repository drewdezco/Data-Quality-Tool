...
class DataQualityChecker:
    ...
    # -------- JSON Rules Loader --------
    def run_rules_from_json(self, json_file):
    ...
    # -------- HTML Report --------
    def generate_data_docs(self, output_path="data_docs.html", title="Data Quality Report", dataset_name="Dataset"):
        ...
        <div class="field-summary-filter" style="margin-bottom: 18px; position: relative;">
            <button style="margin-top: 20px;" id="fieldSummaryDropdownBtn" class="dropdown-btn">Filter columns &#x25BC;</button>
    ...
    def _generate_field_summary_html(self):
        """Generate HTML for field summary tab with enhanced statistics and quality scores"""
        ...
        for column in self.df.columns:
            ...
            html_parts.append(f"""
            <div class="field-section" data-column="{column}">
                <h3>Column: {column}</h3>
...
# -------------------------
# Example test cases follow
# -------------------------

# Create sample data for testing
sample_data = pd.DataFrame({
    'name': ['John', 'Jane', 'Bob', 'Alice', None],
    'email': ['john@example.com', 'jane@example.com', 'invalid-email', 'alice@example.com', 'bob@test.com'],
    'age': [25, 30, 35, 28, 45],
    'department': ['IT', 'HR', 'IT', 'Finance', 'IT']
})


# Create comprehensive dataset with 50 columns and 125 rows
def create_comprehensive_dataset():
    """Create a comprehensive dataset with 50 columns and 125 rows for testing"""
    np.random.seed(42)  # For reproducible results

    # Generate 125 rows
    n_rows = 125

    # Column definitions with various data types and quality issues
    columns_data = {
        # Personal Information (10 columns)
        'employee_id': [f"EMP{i:03d}" for i in range(1, n_rows + 1)],
        'first_name': [f"Name{i}" for i in range(1, n_rows + 1)],
        'last_name': [f"Last{i}" for i in range(1, n_rows + 1)],
        'email': [f"user{i}@company.com" for i in range(1, n_rows + 1)],
        'phone': [f"+1-555-{i:03d}-{i:04d}" for i in range(1, n_rows + 1)],
        'date_of_birth': pd.date_range('1980-01-01', periods=n_rows, freq='D'),
        'ssn': [f"{i:03d}-{i:02d}-{i:04d}" for i in range(1, n_rows + 1)],
        'address': [f"{i} Main St, City{i}, ST {i:02d}000" for i in range(1, n_rows + 1)],
        'city': ["New York", "Los Angeles", "Chicago", "Houston", "Phoenix"] * 25,
        'state': ["NY", "CA", "IL", "TX", "AZ"] * 25,

        # Employment Information (10 columns)
        'hire_date': pd.date_range('2015-01-01', periods=n_rows, freq='D'),
        'department': ["IT", "HR", "Finance", "Marketing", "Sales"] * 25,
        'job_title': ["Engineer", "Manager", "Analyst", "Director", "Coordinator"] * 25,
        'salary': np.random.normal(75000, 25000, n_rows).round(2),
        'bonus': np.random.normal(5000, 2000, n_rows).round(2),
        'manager_id': [f"MGR{i:03d}" for i in range(1, n_rows + 1)],
        'employment_status': ["Full-time", "Part-time", "Contract"] * 41 + ["Full-time", "Part-time"],
        'work_location': ["Office", "Remote", "Hybrid"] * 41 + ["Office", "Remote"],
        'employee_type': ["Regular", "Temporary", "Intern"] * 41 + ["Regular", "Temporary"],
        'contract_end_date': list(pd.date_range('2024-01-01', periods=n_rows, freq='D')),

        # Performance Metrics (10 columns)
        'performance_rating': np.random.choice([1, 2, 3, 4, 5], n_rows, p=[0.1, 0.2, 0.4, 0.2, 0.1]),
        'attendance_rate': np.random.uniform(0.85, 1.0, n_rows).round(3),
        'projects_completed': np.random.poisson(8, n_rows),
        'customer_satisfaction': np.random.uniform(3.0, 5.0, n_rows).round(2),
        'training_hours': np.random.exponential(20, n_rows).round(1),
        'certifications': np.random.poisson(2, n_rows),
        'innovation_score': np.random.uniform(0, 100, n_rows).round(1),
        'teamwork_rating': np.random.uniform(1, 5, n_rows).round(2),
        'leadership_score': np.random.uniform(0, 100, n_rows).round(1),
        'productivity_index': np.random.uniform(0.6, 1.2, n_rows).round(3),

        # Financial Data (10 columns)
        'base_salary': np.random.normal(70000, 20000, n_rows).round(2),
        'overtime_pay': np.random.exponential(2000, n_rows).round(2),
        'commission': np.random.uniform(0, 15000, n_rows).round(2),
        'benefits_cost': np.random.normal(12000, 3000, n_rows).round(2),
        'tax_withholding': np.random.uniform(0.15, 0.35, n_rows).round(3),
        'retirement_contribution': np.random.uniform(0, 0.15, n_rows).round(3),
        'health_insurance_premium': np.random.normal(500, 150, n_rows).round(2),
        'dental_insurance_premium': np.random.normal(50, 20, n_rows).round(2),
        'vision_insurance_premium': np.random.normal(30, 10, n_rows).round(2),
        'life_insurance_amount': np.random.choice([50000, 100000, 200000, 500000], n_rows),

        # System/Technical Data (10 columns)
        'user_id': [f"USER{i:04d}" for i in range(1, n_rows + 1)],
        'login_count': np.random.poisson(150, n_rows),
        'last_login': pd.date_range('2024-01-01', periods=n_rows, freq='H'),
        'system_access_level': np.random.choice([1, 2, 3, 4, 5], n_rows),
        'password_last_changed': pd.date_range('2023-01-01', periods=n_rows, freq='D'),
        'two_factor_enabled': np.random.choice([True, False], n_rows, p=[0.8, 0.2]),
        'vpn_usage_hours': np.random.exponential(40, n_rows).round(1),
        'data_usage_gb': np.random.exponential(50, n_rows).round(2),
        'security_incidents': np.random.poisson(0.5, n_rows),
        'backup_frequency_days': np.random.choice([1, 7, 30], n_rows, p=[0.6, 0.3, 0.1])
    }
    ...
    for col in ['first_name', 'contract_end_date', 'last_login']:
        null_indices = np.random.choice(df.index, size=int(n_rows * 0.05), replace=False)
        df.loc[null_indices, col] = None

    # Add some duplicate values
    df.loc[10:15, 'email'] = df.loc[10, 'email']
    df.loc[20:25, 'phone'] = df.loc[20, 'phone']

    # Add some invalid data
    df.loc[30:35, 'email'] = "invalid-email-format"
    df.loc[40:45, 'phone'] = "not-a-phone"
    df.loc[50:55, 'ssn'] = "000-00-0000"

    # Add some out-of-range values
    df.loc[60:65, 'age'] = np.random.choice([-5, 150, 200], 6)
    df.loc[70:75, 'salary'] = np.random.choice([-1000, 1000000, 9999999], 6)
...
checker = DataQualityChecker(comprehensive_data)

# Example 1 - Mostly Healthy
checker.results = [
    {"column": "employee_id", "rule": "not null", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0}},
    {"column": "employee_id", "rule": "unique", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0}},
    {"column": "first_name", "rule": "not null", "success_rate": 95.2,
     "details": {"total": 125, "passed": 119, "failed": 6}},
    {"column": "email", "rule": "matches regex", "success_rate": 92.8,
     "details": {"total": 125, "passed": 116, "failed": 9, "pattern": r"^[^@]+@[^@]+\.[^@]+$"}},
    {"column": "phone", "rule": "matches regex", "success_rate": 98.4,
     "details": {"total": 125, "passed": 123, "failed": 2, "pattern": r"^\+1-555-\d{3}-\d{4}$"}},
    {"column": "ssn", "rule": "matches regex", "success_rate": 96.8,
     "details": {"total": 125, "passed": 121, "failed": 4, "pattern": r"^\d{3}-\d{2}-\d{4}$"}},
    {"column": "department", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0,
                 "allowed_values": ["IT", "HR", "Finance", "Marketing", "Sales"]}},
    {"column": "employment_status", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "allowed_values": ["Full-time", "Part-time", "Contract"]}},
    {"column": "salary", "rule": "in range 30000-200000", "success_rate": 95.2,
     "details": {"total": 125, "passed": 119, "failed": 6, "range": (30000, 200000)}},
    {"column": "performance_rating", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "allowed_values": [1, 2, 3, 4, 5]}},
    {"column": "attendance_rate", "rule": "in range 0.8-1.0", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0.8, 1.0)}},
    {"column": "hire_date", "rule": "in date range 2010-2024", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": ("2010-01-01", "2024-12-31")}},
    {"column": "date_of_birth", "rule": "in date range 1950-2005", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": ("1950-01-01", "2005-12-31")}},
    {"column": "email", "rule": "unique", "success_rate": 95.2,
     "details": {"total": 125, "passed": 119, "failed": 6}},
    {"column": "phone", "rule": "unique", "success_rate": 98.4,
     "details": {"total": 125, "passed": 123, "failed": 2}},
    {"column": "ssn", "rule": "unique", "success_rate": 96.8,
     "details": {"total": 125, "passed": 121, "failed": 4}},
    {"column": "user_id", "rule": "unique", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0}},
    {"column": "manager_id", "rule": "not null", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0}},
    {"column": "contract_end_date", "rule": "not null", "success_rate": 92.0,
     "details": {"total": 125, "passed": 115, "failed": 10}},
    {"column": "last_login", "rule": "not null", "success_rate": 95.2,
     "details": {"total": 125, "passed": 119, "failed": 6}},
    {"column": "base_salary", "rule": "in range 30000-200000", "success_rate": 95.2,
     "details": {"total": 125, "passed": 119, "failed": 6, "range": (30000, 200000)}},
    {"column": "bonus", "rule": "in range 0-50000", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 50000)}},
    {"column": "commission", "rule": "in range 0-30000", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 30000)}},
    {"column": "projects_completed", "rule": "in range 0-50", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 50)}},
    {"column": "customer_satisfaction", "rule": "in range 1.0-5.0", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (1.0, 5.0)}},
    {"column": "training_hours", "rule": "in range 0-200", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 200)}},
    {"column": "certifications", "rule": "in range 0-10", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 10)}},
    {"column": "innovation_score", "rule": "in range 0-100", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 100)}},
    {"column": "teamwork_rating", "rule": "in range 1.0-5.0", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (1.0, 5.0)}},
    {"column": "leadership_score", "rule": "in range 0-100", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 100)}},
    {"column": "productivity_index", "rule": "in range 0.5-1.5", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0.5, 1.5)}},
    {"column": "overtime_pay", "rule": "in range 0-20000", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 20000)}},
    {"column": "benefits_cost", "rule": "in range 5000-25000", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (5000, 25000)}},
    {"column": "tax_withholding", "rule": "in range 0.1-0.4", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0.1, 0.4)}},
    {"column": "retirement_contribution", "rule": "in range 0-0.2", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 0.2)}},
    {"column": "health_insurance_premium", "rule": "in range 200-1000", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (200, 1000)}},
    {"column": "dental_insurance_premium", "rule": "in range 20-150", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (20, 150)}},
    {"column": "vision_insurance_premium", "rule": "in range 10-100", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (10, 100)}},
    {"column": "life_insurance_amount", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "allowed_values": [50000, 100000, 200000, 500000]}},
    {"column": "login_count", "rule": "in range 0-1000", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 1000)}},
    {"column": "system_access_level", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "allowed_values": [1, 2, 3, 4, 5]}},
    {"column": "two_factor_enabled", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "allowed_values": [True, False]}},
    {"column": "vpn_usage_hours", "rule": "in range 0-200", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 200)}},
    {"column": "data_usage_gb", "rule": "in range 0-500", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 500)}},
    {"column": "security_incidents", "rule": "in range 0-10", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "range": (0, 10)}},
    {"column": "backup_frequency_days", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "allowed_values": [1, 7, 30]}},
    {"column": "work_location", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "allowed_values": ["Office", "Remote", "Hybrid"]}},
    {"column": "employee_type", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "allowed_values": ["Regular", "Temporary", "Intern"]}},
    {"column": "job_title", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0,
                 "allowed_values": ["Engineer", "Manager", "Analyst", "Director", "Coordinator"]}},
    {"column": "city", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0,
                 "allowed_values": ["New York", "Los Angeles", "Chicago", "Houston", "Phoenix"]}},
    {"column": "state", "rule": "in allowed set", "success_rate": 100.0,
     "details": {"total": 125, "passed": 125, "failed": 0, "allowed_values": ["NY", "CA", "IL", "TX", "AZ"]}},
]
checker.generate_data_docs(output_path="comprehensive_report.html", title="Comprehensive Data Quality Report",
                           dataset_name="Employee Dataset (125 rows, 50 columns)")

# Create a comprehensive test DataFrame with all data types and quality issues
def create_comprehensive_test_data():
    """Create a test DataFrame with all data types and various quality issues"""
    import numpy as np
    import pandas as pd
    ...
    # Create comprehensive test data with various data types and issues
    test_data = pd.DataFrame({
        # String columns with various issues
        'employee_id': [f"EMP{i:03d}" for i in range(1, n_rows + 1)],
        'first_name': ['John', 'Jane', 'Bob', 'Alice', 'Charlie', 'Diana', 'Eve', 'Frank', 
                      'Grace', 'Henry', 'Ivy', 'Jack', 'Kate', 'Liam', 'Mia', 'Noah', 
                      'Olivia', 'Paul', 'Quinn', None],  # One null value
        'email': ['john@company.com', 'jane@company.com', 'invalid-email', 'alice@company.com',
                 'charlie@company.com', 'diana@company.com', 'eve@company.com', 'frank@company.com',
                 'grace@company.com', 'henry@company.com', 'ivy@company.com', 'jack@company.com',
                 'kate@company.com', 'liam@company.com', 'mia@company.com', 'noah@company.com',
                 'olivia@company.com', 'paul@company.com', 'quinn@company.com', 'robert@company.com'],
        'phone': ['+1-555-001-0001', '+1-555-002-0002', '+1-555-003-0003', '+1-555-004-0004',
                 '+1-555-005-0005', '+1-555-006-0006', '+1-555-007-0007', '+1-555-008-0008',
                 '+1-555-009-0009', '+1-555-010-0010', '+1-555-011-0011', '+1-555-012-0012',
                 '+1-555-013-0013', '+1-555-014-0014', '+1-555-015-0015', '+1-555-016-0016',
                 '+1-555-017-0017', '+1-555-018-0018', '+1-555-019-0019', '+1-555-020-0020'],
        ...
        # Numeric columns with various issues
        'age': [25, 30, 35, 28, 45, 22, 38, 42, 29, 33, 27, 31, 26, 34, 39, 41, 23, 37, 32, 36],
        'salary': [50000, 60000, 75000, 55000, 90000, 45000, 80000, 85000, 52000, 65000,
                  48000, 70000, 53000, 68000, 82000, 88000, 47000, 78000, 62000, 72000],
        'bonus': [5000, 6000, 7500, 5500, 9000, 4500, 8000, 8500, 5200, 6500,
                 4800, 7000, 5300, 6800, 8200, 8800, 4700, 7800, 6200, 7200],
        'performance_rating': [4.2, 3.8, 4.5, 3.9, 4.7, 3.5, 4.3, 4.1, 3.7, 4.0,
                              4.4, 3.6, 4.6, 3.4, 4.8, 3.2, 4.9, 3.1, 4.1, 3.9],
        
        # Date columns with various issues
        'hire_date': dates,
        'birth_date': [base_date - timedelta(days=365*25), base_date - timedelta(days=365*30),
                      base_date - timedelta(days=365*35), base_date - timedelta(days=365*28),
                      base_date - timedelta(days=365*45), base_date - timedelta(days=365*22),
                      base_date - timedelta(days=365*38), base_date - timedelta(days=365*42),
                      base_date - timedelta(days=365*29), base_date - timedelta(days=365*33),
                      base_date - timedelta(days=365*27), base_date - timedelta(days=365*31),
                      base_date - timedelta(days=365*26), base_date - timedelta(days=365*34),
                      base_date - timedelta(days=365*39), base_date - timedelta(days=365*41),
                      base_date - timedelta(days=365*23), base_date - timedelta(days=365*37),
                      base_date - timedelta(days=365*32), base_date - timedelta(days=365*36)],
        'last_login': [base_date + timedelta(hours=i) for i in range(n_rows)],
        
        # Categorical columns with various issues
        'department': ['IT', 'HR', 'Finance', 'Marketing', 'Sales', 'IT', 'HR', 'Finance', 
                      'Marketing', 'Sales', 'IT', 'HR', 'Finance', 'Marketing', 'Sales',
                      'IT', 'HR', 'Finance', 'Marketing', 'Sales'],
        'job_title': ['Engineer', 'Manager', 'Analyst', 'Director', 'Coordinator', 'Engineer',
                     'Manager', 'Analyst', 'Director', 'Coordinator', 'Engineer', 'Manager',
                     'Analyst', 'Director', 'Coordinator', 'Engineer', 'Manager', 'Analyst',
                     'Director', 'Coordinator'],
        'employment_status': ['Full-time', 'Part-time', 'Contract', 'Full-time', 'Part-time',
                            'Full-time', 'Part-time', 'Contract', 'Full-time', 'Part-time',
                            'Full-time', 'Part-time', 'Contract', 'Full-time', 'Part-time',
                            'Full-time', 'Part-time', 'Contract', 'Full-time', 'Part-time'],
        'work_location': ['Office', 'Remote', 'Hybrid', 'Office', 'Remote', 'Office', 'Remote',
                         'Hybrid', 'Office', 'Remote', 'Office', 'Remote', 'Hybrid', 'Office',
                         'Remote', 'Office', 'Remote', 'Hybrid', 'Office', 'Remote'],
        
        # Boolean columns
        'is_manager': [True, False, True, False, True, False, True, False, True, False,
                      True, False, True, False, True, False, True, False, True, False],
        'has_benefits': [True, True, False, True, True, False, True, True, False, True,
                        True, False, True, True, False, True, True, False, True, True],
        
        # Float columns with precision
        'attendance_rate': [0.95, 0.88, 0.92, 0.97, 0.85, 0.94, 0.89, 0.91, 0.96, 0.87,
                           0.93, 0.90, 0.98, 0.86, 0.99, 0.84, 0.97, 0.88, 0.92, 0.95],
        'customer_satisfaction': [4.2, 3.8, 4.5, 3.9, 4.7, 3.5, 4.3, 4.1, 3.7, 4.0,
                                 4.4, 3.6, 4.6, 3.4, 4.8, 3.2, 4.9, 3.1, 4.1, 3.9]
    })
    
    # Introduce some data quality issues
    # Add some null values
    test_data.loc[5, 'first_name'] = None
    test_data.loc[10, 'email'] = None
    test_data.loc[15, 'phone'] = None
    
    # Add some invalid data
    test_data.loc[2, 'email'] = 'invalid-email-format'
    test_data.loc[7, 'phone'] = 'not-a-phone-number'
    test_data.loc[12, 'ssn'] = 'invalid-ssn'
    
    # Add some out-of-range values
    test_data.loc[3, 'age'] = 150  # Invalid age
    test_data.loc[8, 'salary'] = -5000  # Negative salary
    test_data.loc[13, 'performance_rating'] = 6.0  # Rating > 5
    
    # Add some duplicates
    test_data.loc[4, 'employee_id'] = 'EMP001'  # Duplicate ID
    test_data.loc[9, 'email'] = 'john@company.com'  # Duplicate email
    
    # Add some invalid categories
    test_data.loc[6, 'department'] = 'Invalid-Dept'
    test_data.loc[11, 'job_title'] = 'Invalid-Title'
    
    return test_data

# Create the comprehensive test data
comprehensive_test_data = create_comprehensive_test_data()

# Create a comprehensive data quality checker
checker_comprehensive = DataQualityChecker(comprehensive_test_data)

# Run ALL validation types on the comprehensive dataset
print("Running comprehensive data quality validations...")

# String validations
checker_comprehensive.expect_column_values_to_not_be_null("first_name")
checker_comprehensive.expect_column_values_to_not_be_null("email")
checker_comprehensive.expect_column_values_to_not_be_null("phone")
checker_comprehensive.expect_column_values_to_not_be_null("ssn")

checker_comprehensive.expect_column_values_to_be_unique("employee_id")
checker_comprehensive.expect_column_values_to_be_unique("email")
checker_comprehensive.expect_column_values_to_be_unique("phone")
checker_comprehensive.expect_column_values_to_be_unique("ssn")

checker_comprehensive.expect_column_values_to_match_regex("email", r"^[^@]+@[^@]+\.[^@]+$")
checker_comprehensive.expect_column_values_to_match_regex("phone", r"^\+1-555-\d{3}-\d{4}$")
checker_comprehensive.expect_column_values_to_match_regex("ssn", r"^\d{3}-\d{2}-\d{4}$")

checker_comprehensive.expect_column_values_to_be_in_set("department", ["IT", "HR", "Finance", "Marketing", "Sales"])
checker_comprehensive.expect_column_values_to_be_in_set("job_title", ["Engineer", "Manager", "Analyst", "Director", "Coordinator"])
checker_comprehensive.expect_column_values_to_be_in_set("employment_status", ["Full-time", "Part-time", "Contract"])
checker_comprehensive.expect_column_values_to_be_in_set("work_location", ["Office", "Remote", "Hybrid"])

# Numeric validations
checker_comprehensive.expect_column_values_to_not_be_null("age")
checker_comprehensive.expect_column_values_to_not_be_null("salary")
checker_comprehensive.expect_column_values_to_not_be_null("bonus")
checker_comprehensive.expect_column_values_to_not_be_null("performance_rating")

checker_comprehensive.expect_column_values_to_be_in_range("age", 18, 65)
checker_comprehensive.expect_column_values_to_be_in_range("salary", 30000, 150000)
checker_comprehensive.expect_column_values_to_be_in_range("bonus", 0, 20000)
checker_comprehensive.expect_column_values_to_be_in_range("performance_rating", 1.0, 5.0)
checker_comprehensive.expect_column_values_to_be_in_range("attendance_rate", 0.0, 1.0)
checker_comprehensive.expect_column_values_to_be_in_range("customer_satisfaction", 1.0, 5.0)

# Date validations
checker_comprehensive.expect_column_values_to_not_be_null("hire_date")
checker_comprehensive.expect_column_values_to_not_be_null("birth_date")
checker_comprehensive.expect_column_values_to_not_be_null("last_login")

# Date range validations (hire dates should be recent, birth dates should be reasonable)
from datetime import datetime, timedelta
current_date = datetime.now()
min_hire_date = current_date - timedelta(days=365*10)  # 10 years ago
max_hire_date = current_date
min_birth_date = current_date - timedelta(days=365*80)  # 80 years ago
max_birth_date = current_date - timedelta(days=365*18)  # 18 years ago

checker_comprehensive.expect_column_values_to_be_in_date_range("hire_date", min_hire_date, max_hire_date)
checker_comprehensive.expect_column_values_to_be_in_date_range("birth_date", min_birth_date, max_birth_date)

# Boolean validations
checker_comprehensive.expect_column_values_to_not_be_null("is_manager")
checker_comprehensive.expect_column_values_to_not_be_null("has_benefits")

checker_comprehensive.expect_column_values_to_be_in_set("is_manager", [True, False])
checker_comprehensive.expect_column_values_to_be_in_set("has_benefits", [True, False])

# Generate comprehensive report
checker_comprehensive.generate_data_docs(
    output_path="comprehensive_test_report.html", 
    title="Comprehensive Data Quality Test Report",
    dataset_name="Test Dataset (20 rows, 18 columns - All Data Types)"
)

print("Comprehensive test report generated: comprehensive_test_report.html")
